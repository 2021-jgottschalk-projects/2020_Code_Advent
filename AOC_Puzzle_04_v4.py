...
# functions go here
def is_ok(pattern, string):

    to_find = re.search(pattern, string)

    if to_find is not None:
        found_it = to_find.group()
        return found_it[4:]
    else:
        return ""


def check_int(int_to_check, var_min, var_max):
    try:
        int_to_check = int(int_to_check)
        if var_min <= int_to_check <= var_max:
            return 1
        else:
            return 0
...
maybe_valid = ['eyr:2033,hgt:177cm pid:173cm,ecl:utc byr:2029 hcl:#efcc98 iyr:2023',
...
hgt_pattern = r"hgt:\d{,3}(cm|in)"
hcl_pattern = r"hcl:#(\d|[a-f]){6}"
ecl_pattern = r"ecl:(amb|blu|brn|gry|grn|hzl|oth)"
pid_pattern = r"pid:\d{9}"

for item in maybe_valid:
    print()

    points = 0

    # check byr
    byr = is_ok(byr_pattern, item)
    byr_ok = check_int(byr, 1920, 2002)
    if byr_ok == 0:
        continue

    iyr = is_ok(iyr_pattern, item)
    iyr_ok = check_int(iyr, 2010, 2020)
    if iyr_ok == 0:
        continue

    eyr = is_ok(eyr_pattern, item)
    print(eyr)
    eyr_ok = check_int(eyr, 2020, 2030)
    print(eyr_ok)
    if eyr_ok == 0:
        continue


    hgt = is_ok(hgt_pattern, item)

    hgt_break = len(hgt) - 2
    hgt_num = hgt[:hgt_break]
    hgt_unit = hgt[hgt_break:]

    if hgt_unit == "cm":
        hgt_ok = check_int(hgt_num, 150, 193)
    elif hgt_unit == "in":
        hgt_ok = check_int(hgt_num, 59, 76)
    else:
        continue
    ...
    hcl_ok = is_ok(hcl_pattern, item)
    if hcl_ok == 0:
        continue

    ecl_ok = is_ok(ecl_pattern, item)
    if ecl_ok != "":
        ecl_ok = 1
    else:
        continue
